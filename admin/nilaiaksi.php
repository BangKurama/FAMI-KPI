		...
		<form action="nilaiproses.php?proses=prosestambah" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<select class="form-control" name="id_alternatif">
					...
					<?php
					$b1 = mysqli_query($conn,"SELECT * FROM tbl_alternatif order by id_alternatif"); while($b=mysqli_fetch_array($b1)){ ?>

						<option value="<?php echo $b['id_alternatif']?>"><?php echo $b['id_alternatif']?> - <?php echo $b['nama_alternatif']?></option>
	...
	<div class="panel panel-container">
		...
		<form action="nilaiproses.php?proses=prosesubah" method="post" enctype="multipart/form-data">
				...
				$id_alternatif=$_GET['id_alternatif'];
					$a1 = mysqli_query($conn,"SELECT * FROM tbl_alternatif WHERE id_alternatif='$id_alternatif'"); 
					$a=mysqli_fetch_array($a1);

				?>
				<select class="form-control" name="id_alternatif">
					<option selected value="<?php echo $a['id_alternatif']?>"><?php echo $a['id_alternatif']?> - <?php echo $a['nama_alternatif']?></option>
			...
			while($baris = mysqli_fetch_array($hasil)){
				...
				$hasil2 = mysqli_query($conn,"SELECT * FROM tbl_nilai WHERE id_kriteria='".$idK."' AND id_alternatif='".$id_alternatif."'"); 
			...
			<div class="modal-footer">
				<a href="nila.php" class="btn btn-success">KEMBALI</a>
